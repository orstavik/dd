 ...
 * 
 * Usage:
 * class AttrClass extends AttrMutation {
 *  run(mr) { eventLoop.dispatch(Object.assign(new Event("class"), {oldValue: mr.oldValue})) }
 *  get settings() { return { attributeFilter: ["class"], attributesOldValue: true }; }
 * }
 * 
 * You should override get settings() and run(mr) to suit your needs.
 * 
 * att!! You can also override get target().
 * You can for example set this.ownerElement.parentElement or .previousSiblingElement and observe their changes.
 * But consider lob! when you do this!
 */
class AttrMutation extends AttrCustom {
  upgrade() {
    const observer = new MutationObserver(mrs => this.run(mrs[0]));
