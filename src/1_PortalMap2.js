...
function memoizeAsync(fn, maxLimit = 10000) {
  ...
  return function (strArg) {
    ...
    if (res instanceof Promise)
      res = res.then(
        result => cacheRef.cache[strArg] = result,
        cause => cacheRef.cache[strArg] = new Error(fn.name + ": " + strArg, { cause })
      );
